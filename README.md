...
Проверяет отсутствие активного подключения для авторизованного пользователя. Если подключений нет, возвращает `null`, если подключение было найдено, выбрасывает ошибку с внутр. кодом 7 (см. ниже).
...
|400|7|Пользователь уже имеет активное подключение|